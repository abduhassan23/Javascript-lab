 ...
 function recalculate(modelName,duration)
{
    ...
    if (modelName == "Model XYZ") 
        cc= 100 * duration;
         ...
         document.getElementById("calculated-cost").innerText=cc;
...
// INSERT YOUR CODE HERE
let switchModel = getElementById("model-button");

function changeModel(modelName) {
    let modelText = document.getElementById("model-text");
    if (modelName == "Model XYZ") {
        modelButton.onclick=modelText.innerHTML="Model CPRG";
    }
    else { switchModel.onclick=modelText.innerHTML="Model XYZ";

    }
    recalculate(modelName, duration);
    }
   function modelButton.addEventListener("click", changeModel);
...
function changeDuration(duration){
    duration.innerHTML=prompt("Please set duration.");
